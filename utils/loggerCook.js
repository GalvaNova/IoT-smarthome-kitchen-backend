// ===============================================
// utils/loggerCook.js (Final Revised)
// ===============================================
...
// ===============================================
const LOG_DIR = path.join(__dirname, "../logs/cook");
...
function initLogFile() {
  ...
  if (today !== currentDate) {
    currentDate = today;
    logFile = path.join(LOG_DIR, `cooklog_${today}.csv`);

    if (!fs.existsSync(logFile)) {
      const header =
        "timestamp_backend_cook,timestamp_reasoner_cook,timestamp_iot_cook,temp,flame,gas,distance,buzzerStatus,exhaustStatus,cookingStatus,reasoningTime,endToEndTime,backendTime\n";
...
function getLocalTimestamp() {
  ...
  // Gabungkan tanpa koma agar CSV tetap satu kolom
  return `${date} ${time}`;
}
...
async function writeLog(data) {
  ...
  const line =
    [
      data.timestamp_backend_cook || getLocalTimestamp(),
      data.timestamp_reasoner_cook || "-",
      data.timestamp_iot_cook || "-",
      data.temp ?? 0,
      data.flame ?? 0,
      data.gas ?? 0,
      data.distance ?? 0,
      data.buzzerStatus || "unknown",
      data.exhaustStatus || "unknown",
      data.cookingStatus || "st_cookNO",
